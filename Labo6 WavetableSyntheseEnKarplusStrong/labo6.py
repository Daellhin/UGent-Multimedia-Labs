...
def synthesize_melody(rtttl: str, base_duration: int, base_octave: int, bpm: int, fs: int, fade_in=True):
    ...
    for note in notes:
        ...
        if pitch == "P":
            ...
            if fade_in:
                ...
                waveform[:fadein_samples] = np.multiply(waveform[:fadein_samples], fade_in_envelope)
        waveforms.extend(waveform)

    return waveforms


def plot_waveforms(sounds: list[list[float]], duration: int, frequencies: list[float], title="Image"):
...
def plot_spectrograms(sounds: list[list[float]], frequencies: list[float], fs: int, title="Image"):
